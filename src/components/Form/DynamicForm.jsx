...
const DynamicForm = ({ formData, onSubmit, formButtons, formId="main" }) => {
    ...
    formData.forEach((field) => {
      if (field.dependentField) {
        setFieldVisibility((prev) => ({
          ...prev,
          [field.name]:
            formValues[field.dependentField] === field.dependentValue,
        }));
      }
    });
  }, [formValues, formData]);
  ...
  const handleFileChange = useCallback((event) => {
    ...
    if (selectedFile && ["image/jpeg", "image/png", "application/pdf"].includes(selectedFile.type)) {
      setFormValues((prevValues) => ({
        ...prevValues,
        [name]: selectedFile,
      }));
        ...
        {formData.map(
          (field, index) =>
            (!field.dependentField || fieldVisibility[field.name]) && (
              <div
                key={index}
                className={`col-md-${12 / field.grid} ${
                  field.isHidden ? "d-none" : ""
                }`}
              >
  ...
  function renderFormField(field) {
    ...
    switch (type) {
            ...
            >
              ...
              {!disabled && (previewUrl ? (
                ...
                <div>
                  ...
                  {
                    !isInline && <p className="text-primary" style={{ fontSize: "12px", marginTop: "4px" }} > (Max File Size: 1MB)</p>
                  }
                </div>
              ))}
            </label>
      ...
      case "rowHeader":
        return (
          <>
            <p className="header" style={{fontSize:"17px"}}>{label}</p>
      ...
      case "button":
        return (
          <>
            <button className={classNames} type="button" onClick={onClick} style={{display:`${show?"block":"none"}`}}>{label}</button>
          </>
      );
      default:
        return (
